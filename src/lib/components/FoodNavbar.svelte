<script>
    ...
    const routesWithActive = $derived(
        routesAndNamesFood.map(item => ({
            ...item,
            markActive: item.route === page.url.pathname
        }))
    );
</script>

<NavbarLayout routesAndNames={routesWithActive}></NavbarLayout>